...
/**
 * Default builder for field definitions. It uses a {@link FieldGroupFieldFactory} as a source for building components, unless it is a password or free text
 * field.
 * 
 * @author miki
 *
...
public class DefaultFormBuilderFieldFactory implements FormBuilderFieldFactory {
  ...
  public Field<?> buildField(Class<?> fieldType, String fieldIdentifier, String fieldCaption, Object... additionalInformation) {
    ...
    if(info.contains(FieldInformation.PASSWORD))
      ...
      result = new TextArea();
    else result = this.getFieldFactory().createField(fieldType, Field.class);
  ...
  /**
   * Returns the currently used field factory.
   * 
   * @return Field factory used by the form builder.
   */
  ...
  /**
   * Sets the new field factory.
   * 
   * @param fieldFactory
   *          Field factory to use for building.
   */
